...
public class HospitalService {
    ...
    public Hospital findNearestHospital(String specialty, double patientLatitude, double patientLongitude) {
        ...
        return hospitals.stream()
                .filter(hospital -> hospital.getSpecializations().stream().anyMatch(s -> s.getName().equalsIgnoreCase(specialty)))
                .min(Comparator.comparingDouble(hospital -> calculateDistance(patientLatitude, patientLongitude, hospital.getLatitude(), hospital.getLongitude())))
                .orElse(null);
    }
